...
- [[#vim-plug-requires-git][Vim plug (requires git)]]
  ...
  - [[#declare-the-list-of-plugins][Declare the list of plugins]]
    ...
    - [[#inspecthivim-detection-of-syntax-group-under-cursor][inspecthi.vim (detection of syntax group under cursor)]]
    - [[#zplugin-vim-syntax-syntax-highlight-zplugin-commands][zplugin vim syntax (syntax highlight zplugin commands)]]
...
I've been thinking for a while that it would be nice to be able to see what syntax group a string under the cursor uses (something akin to emacs's ~decribe-face~).[[https://github.com/cocopon/inspecthi.vim][This plugin]] does this.
#+begin_src vim
Plug 'cocopon/inspecthi.vim'
#+end_src
*** zplugin vim syntax (syntax highlight zplugin commands)
Since I use zsh and the [[https://github.com/zdharma/zplugin][zplugin]] zsh plugin manager, it only made sense that I'd want to see zplugin commands syntax highlighted. [[https://github.com/zplugin/zplugin-vim-syntax][This plugin]] does that.
#+begin_src vim
...
#+end_src
Show the number of current and total search results in the commandline (see [[https://github.com/vim/vim/commit/9dfa3139198b38b28673e251a3756430065914e9][here]]). This option appears from vim version 8.1.1270 onward.
#+begin_src vim
set shortmess-=S
#+end_src
